...
The same Anthropic credential does the domain-research analysis. No second service and no second key are needed:
...
4. Repeat for the **Analyse Offering Profile** node in `53 - TOOL - start_paid_domain_research`.

Free domain research reads three destinations and nothing else: the researched domain's own public home page, the Anthropic API, and the local chat gateway at `http://127.0.0.1:3000`. If you intentionally changed the chat port, update those reviewed node URLs before publishing.
...
| **complete_domain_research** | Reports what that conversation-bound job saved, or says plainly that it never finished |
...
validation path, the optional DataForSEO credential selection, and the health
workflow. It calls neither Claude nor DataForSEO and displays no credential
values.
